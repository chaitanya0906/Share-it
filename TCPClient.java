...
/*
	Code Written By : Chaitanya Bhutada	(the one who aims to a Red Coder one Day :-))
*/
class TCPClient extends JFrame implements ActionListener, MouseListener {
	JPanel panel;
	JLabel title, subT, msg, error, servFiles,backGround;
	Font font,labelfont;
	JTextField txt;
	JButton up, down;
	String dirName;
	...
	public TCPClient(String dir, String host, int port) {
		...
		subT.setOpaque(true);
		subT.setBackground(Color.RED);
		subT.setForeground(Color.BLACK);
		subT.setBounds(179, 500, 200, 50);
		panel.add(subT);

		txt = new JTextField();
		txt.setBackground(Color.YELLOW);
		txt.setBounds(400, 500, 500, 50);
		panel.add(txt);
		...
		try {
			...
			servFiles = new JLabel("Files in the Server Directory : *Double Click to copy");
			servFiles.setForeground(Color.BLACK);
			servFiles.setBounds(300, 125, 400, 50);
			panel.add(servFiles);
			
			servFiles = new JLabel("Basic File Transfer from Server to Client's and Vice-Versa Supported");
			servFiles.setForeground(Color.BLACK);
			servFiles.setBounds(300, 165, 500, 50);
			panel.add(servFiles);

			filelist = new JList<>(temp_names);
			JScrollPane scroll = new JScrollPane(filelist);
			scroll.setBounds(270, 240, 530, 230);
